...
def process_sentiment(df):
    ...
    # Generate basic sentiment label for color coding later
...
data_source = st.sidebar.radio("Choose Data Source:", ["Use Demo CSV", "Analyze Real YouTube Video"])
        ...
        if not api_key or not video_url:
            ...
            with st.spinner("Fetching comments from YouTube..."):
                ...
                if raw_df is not None:
                    ...
                    st.session_state['current_df'] = df # Save to session state
else:
    # Use Demo CSV
...
st.markdown("Discover the true emotions behind the comments! Fetch real-time data or explore the demo dataset.")
...
if not df.empty:
    ...
    st.subheader("🔍 Explore the Comments")
    
    # New Beautiful Star Filter
    star_options = ['⭐⭐⭐⭐⭐', '⭐⭐⭐⭐', '⭐⭐⭐', '⭐⭐', '⭐']
    ...
    # Landing page message when no data is loaded
    st.info("👈 Please use the Sidebar to load Demo Data or Analyze a Real YouTube Video!")
